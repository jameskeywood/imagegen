...
images = []
for i in os.scandir('planets'):
  images.append(i.path)

images = tf.data.Dataset.from_tensor_slices(images)
BATCH_SIZE = 8
train_images = images.map(get_ds,num_parallel_calls=tf.data.experimental.AUTOTUNE).batch(BATCH_SIZE).shuffle(buffer_size=363)
...
# train, save 50 figures then repeat 10 times
...
for x in range(100):
  ...
  for y in range(50):
    ...
    plt.savefig(f'{results_dir}{counter}.png', dpi=1200, bbox_inches='tight', pad_inches=0)
